// ChatGPTCodeBox.js
import React, { useState } from 'react';
import { Prism as SyntaxHighlighter } from 'react-syntax-highlighter';

import { CopyToClipboard } from 'react-copy-to-clipboard';
import './ChatGPTCodeBox.css'; // Import CSS file for styling

// npm react-syntax-highlighter
// npm react-copy-to-clipboard
// npm install -D tailwindcss@latest postcss@latest autoprefixer@latest
// npx tailwindcss init

// /* src/index.css */
// @tailwind base;
// @tailwind components;
// @tailwind utilities;


// tailwind.config.js
// './src/**/*.{js,jsx,ts,tsx}', // Scan all JS/TS files in the src folder











const ChatGPTCodeBox = ({ language, code }) => {
    const [copied, setCopied] = useState(false);

    const handleCopy = () => {
        setCopied(true);
        setTimeout(() => {
            setCopied(false);
        }, 1500);
    };

    return (
        <div className="dark bg-gray-950 rounded-md border-token-border-medium text-white code-box w-1/2 mt-10 mb-10 ml-10">
            <div className="flex items-center relative text-token-text-secondary bg-token-main-surface-secondary px-4 py-2 text-xs font-sans justify-between rounded-t-md">
                <span>{language}</span>
                <div className="flex items-center">
                    <CopyToClipboard text={code} onCopy={handleCopy}>
                        <button className="flex gap-1 items-center">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="24"
                                height="24"
                                fill="none"
                                viewBox="0 0 24 24"
                                className="icon-sm"
                            >
                                <path
                                    fill="currentColor"
                                    fillRule="evenodd"
                                    d="M7 5a3 3 0 0 1 3-3h9a3 3 0 0 1 3 3v9a3 3 0 0 1-3 3h-2v2a3 3 0 0 1-3 3H5a3 3 0 0 1-3-3v-9a3 3 0 0 1 3-3h2zm2 2h5a3 3 0 0 1 3 3v5h2a1 1 0 0 0 1-1V5a1 1 0 0 0-1-1h-9a1 1 0 0 0-1 1zM5 9a1 1 0 0 0-1 1v9a1 1 0 0 0 1 1h9a1 1 0 0 0 1-1v-9a1 1 0 0 0-1-1z"
                                    clipRule="evenodd"
                                ></path>
                            </svg>
                            {copied ? 'Copied!' : 'Copy code'}
                        </button>
                    </CopyToClipboard>
                </div>
            </div>
            <div className="overflow-y-auto" dir="ltr">
                <SyntaxHighlighter language={language} style={{}} customStyle={{}} >
                    {code}
                </SyntaxHighlighter>
            </div>
        </div>
    );
};

export default ChatGPTCodeBox;
